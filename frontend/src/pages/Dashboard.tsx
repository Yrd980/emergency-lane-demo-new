  ...
  if (overview.error || system.error) {
    return (
      <StateBlock
        ...
        action={<PrimaryButton icon={RefreshCw} onClick={() => { overview.refetch(); system.refetch(); }}>重试检查</PrimaryButton>}
      />
    );
  }
  ...
  return (
    <div>
      <PageHeader
        eyebrow="OPERATIONS"
        title="应急车道检测工作台"
        description="把接入、事件、复核和系统健康放在同一个闭环里，让现场人员知道现在该做什么。"
        action={
          needsSetup ? (
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link className="text-sm font-semibold text-slate-700 hover:text-slate-950" to="/events">全部事件</Link>
          </div>
        ...
        <section className="space-y-4">
            ...
            <div className="mt-4 space-y-3">
              {system.data.issues.length === 0 ? (
                ...
                system.data.issues.map((issue) => (
                  <div key={issue.code} className="flex gap-3 rounded-lg bg-slate-50 p-3 text-sm">
                    {issue.severity === 'critical' ? <ShieldAlert className="mt-0.5 h-4 w-4 text-red-600" /> : <AlertTriangle className="mt-0.5 h-4 w-4 text-amber-600" />}
                    <div>
          ...
          <div className="rounded-lg border border-slate-200 bg-[#111820] p-4 text-white shadow-sm">
            ...
            <button
              className="mt-4 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-950"
